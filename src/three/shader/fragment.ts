export const fragment = `
	...
	void main() {
		...
		// fix noise uv stretching
		vec2 nUv = uv * vec2(uScreenRes.x / uScreenRes.y, 1.0);
		
		// n = -1 -> 1
		float n = cnoise(vec4(nUv * 12., uOffset, 0.));
		
		// map uProgress from 0->1 to -1->1 to match noise range
		float p = map(uProgress, 0., 1., -1., 1.);
		
		float border = p;
		float final = 1. - smoothstep(border, border + 0.01, n);
		
		// vec4 black = vec4(0,0,0,1);
		// vec4 white = vec4(1,1,1,1);
		// vec4 color = mix(black, white, final);
		vec4 color = mix(currentImage, nextImage, final);
		gl_FragColor = color;
	}
`
